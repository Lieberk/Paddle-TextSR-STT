...
class randomSequentialSampler(Sampler):
    ...
    def __iter__(self):
        ...
        index = paddle.to_tensor([len(self)], dtype='int64').fill_(0)
        for i in range(n_batch):
            random_start = random.randint(0, len(self) - self.batch_size)
            batch_index = random_start + paddle.arange(0, self.batch_size)
            index[i * self.batch_size:(i + 1) * self.batch_size] = batch_index
        # deal with tail
        if tail:
            random_start = random.randint(0, len(self) - self.batch_size)
            tail_index = random_start + paddle.arange(0, tail)
...
class alignCollate_syn(object):
    ...
    def __call__(self, batch):
        ...
        images_hr = paddle.concat([t.unsqueeze(0) for t in images_hr], 0)
        ...
        images_lr = paddle.concat([t.unsqueeze(0) for t in images_lr], 0)
