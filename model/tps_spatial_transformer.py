...
def compute_partial_repr(input_points, control_points):
    ...
    repr_matrix = util.masked_fill(repr_matrix, mask, 0)
    return repr_matrix
...
def build_output_control_points(num_control_points, margins):
    ...
    output_ctrl_pts = paddle.to_tensor(output_ctrl_pts_arr).cast('float32')
...
class TPSSpatialTransformer(nn.Layer):
    ...
    def __init__(self, output_image_size=None, num_control_points=None, margins=None):
        ...
        forward_kernel = paddle.zeros([N + 3, N + 3])
        ...
        forward_kernel[-2:, :N] = target_control_points.transpose([1, 0])
        ...
        Y, X = target_coordinate.split([1, 1], axis=1)
        ...
        target_coordinate_repr = paddle.concat([
            target_coordinate_partial_repr, paddle.ones([HW, 1]), target_coordinate
        ], axis=1)
