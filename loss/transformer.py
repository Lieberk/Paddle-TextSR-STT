...
def attention(query, key, value, mask=None, dropout=None, attention_map=None):
    ...
    if attention_map is not None:
        return paddle.matmul(attention_map, value), attention_map
...
class Decoder(nn.Layer):
    ...
    def forward(self, text, conv_feature, attention_map=None):
        ...
        conv_feature = conv_feature.reshape([b, c, h * w]).transpose([0, 2, 1])
...
class Transformer(nn.Layer):
    ...
    def forward(self, image, text_length, text_input, test=False, attention_map=None):
        ...
        postion_embedding = self.pe(paddle.zeros(text_embedding.shape).cuda()).cuda()  # batch, text_max_length, 512
        ...
        probs_res = paddle.zeros([total_length, get_alphabet_len()], dtype='float32')
